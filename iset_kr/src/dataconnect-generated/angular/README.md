...
## GetPattern
You can execute the `GetPattern` Query using the following Query injector, which is defined in [dataconnect-generated/angular/index.d.ts](./index.d.ts):

```javascript
injectGetPattern(options?: GetPatternOptions, injector?: Injector): CreateDataConnectQueryResult<GetPatternData, undefined>;
```

### Variables
The `GetPattern` Query has no variables.
### Return Type
Recall that calling the `GetPattern` Query injector returns a `CreateDataConnectQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `CreateDataConnectQueryResult.status()` function. You can also check for pending / success / error status using the `CreateDataConnectQueryResult.isPending()`, `CreateDataConnectQueryResult.isSuccess()`, and `CreateDataConnectQueryResult.isError()` functions.

To access the data returned by a Query, use the `CreateDataConnectQueryResult.data()` function. The data for the `GetPattern` Query is of type `GetPatternData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetPattern`'s Query injector

```javascript
... // other imports
import { connectorConfig } from '@dataconnect/generated';
import { injectGetPattern, GetPatternOptions } from '@dataconnect/generated/angular'
import { DataConnect } from '@angular/fire/data-connect';
import { initializeApp } from '@angular/fire/app';
...
export class MyComponent {
  // Since the execution of the query is eager, you don't have to call `execute` to "execute" the Query.
  // Call the Query injector function to get a `CreateDataConnectQueryResult` object which holds the state of your Query.
  query = injectGetPattern();

  // You can also pass in an options function (not object) of type `GetPatternOptions` to the Query injector function.
  options: GetPatternOptions = () => {
    return {
      staleTime: 5 * 1000
    };
  };
  query = injectGetPattern(this.options);
}
```
...
## AddLike
You can execute the `AddLike` Mutation using the `CreateDataConnectMutationResult` object returned by the following Mutation injector (which is defined in [dataconnect-generated/angular/index.d.ts](./index.d.ts)):
```javascript
injectAddLike(options?: AddLikeOptions, injector?: Injector): CreateDataConnectMutationResult<AddLikeData, AddLikeVariables, AddLikeVariables>;
```
...
```
### Return Type
Recall that calling the `AddLike` Mutation injector returns a `CreateDataConnectMutationResult` object. This object holds the state of your Mutation, including whether the Mutation is loading, has completed, or has succeeded/failed, among other things.

To check the status of a Mutation, use the `CreateDataConnectMutationResult.status()` function. You can also check for pending / success / error status using the `CreateDataConnectMutationResult.isPending()`, `CreateDataConnectMutationResult.isSuccess()`, and `CreateDataConnectMutationResult.isError()` functions.

To execute the Mutation, call `CreateDataConnectMutationResult.mutate()`. This function executes the Mutation, but does not return the data from the Mutation. 

To access the data returned by a Mutation, use the `CreateDataConnectMutationResult.data()` function. The data for the `AddLike` Mutation is of type `AddLikeData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `AddLike`'s Mutation injector

```javascript
... // other imports
import { connectorConfig, AddLikeVariables } from '@dataconnect/generated';
import { injectAddLike, AddLikeOptions } from '@dataconnect/generated/angular'
import { DataConnect } from '@angular/fire/data-connect';
import { initializeApp } from '@angular/fire/app';
...
export class MyComponent {
  // Call the Mutation injector function to get a `CreateDataConnectMutationResult` object which holds the state of your Mutation.
  mutation = injectAddLike();

  // You can also pass in a `AddLikeOptions` function (not object) to the Mutation injector function.
  options: AddLikeOptions = () => {
    return {
      onSuccess: () => { console.log('Mutation succeeded!'); }
    };
  };
  mutation = injectAddLike(this.options);
  ...
  executeMutation() {
    ...
    this.mutation.mutate(addLikeVars);
    // Variables can be defined inline as well.
    this.mutation.mutate({ projectId: ..., });

    // You can call `CreateDataConnectMutationResult.mutateAsync()` to execute the Mutation and return a promise with the data returned from the Mutation.
    this.mutation.mutateAsync(addLikeVars);

    // You can also pass in a `AddLikeOptions` object (not function) to `CreateDataConnectMutationResult.mutate()`.
    this.mutation.mutate(addLikeVars, this.options());
  }
}
```
