...
@Component({
  ...
  template: `
    <div class="staff-wrapper" [class.sidebar-hidden]="isCollapsed">
      ...
      <aside class="staff-sidebar" [class.collapsed]="isCollapsed">
        ...
        <div class="sidebar-footer">
          <div class="user-quick-profile">
            <div class="avatar">{{ (currentUser?.name || 'P').charAt(0) }}</div>
            <div class="info">
      ...
      <main class="staff-main">
        <section class="staff-content-area">
  ...
  styles: [`
    ...
    .staff-sidebar.collapsed {
      ...
      .logo-text, .section-label, span, .badge-new, .user-quick-profile .info { display: none; }
...
export class StaffLayoutComponent {
  ...
  getPageTitle(): string {
    // Basic implementation, could be more dynamic with ActivatedRoute
    const path = window.location.pathname;
    if (path.includes('dashboard')) return 'Tableau de bord';
    if (path.includes('students')) return 'Mes Étudiants';
    if (path.includes('schedule')) return 'Emploi du Temps';
    if (path.includes('materials')) return 'Supports de Cours';
